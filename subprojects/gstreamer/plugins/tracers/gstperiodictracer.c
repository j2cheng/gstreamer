...
//#CRESTRON_CHANGE_BEGIN
#if 1
    /* call back is not getting called and hence starting a thread to print the data */	  
    GST_ERROR("Starting thread");
    int res = pthread_create(&(priv->t1), NULL, &thread_function, self);
    if (res)
    {
        GST_ERROR ("Error in creating Tracer Thread %d\n", res);
    }	  
    res = pthread_setname_np(priv->t1, "GstTrac");
    if (res)
    {
        GST_ERROR ("Error in naming the Tracer Thread %d\n", res);
    }
#else	  
...
//#CRESTRON_CHANGE_BEGIN
#if 1
    priv->exit_threard = TRUE;
    sleep(2);  //wait for thread to exit
    pthread_join(priv->t1, NULL);
    GST_ERROR("Thread exited");
#else    
...
static void * thread_function(void *arg)
{
    ...
    priv = GST_PERIODIC_TRACER_PRIVATE (self);
    while(priv->exit_threard == FALSE)
    {	    
    	sleep(1);
        callback_internal(data);
    }
    GST_INFO_OBJECT (self, "Exiting Periodic thread function");
    return NULL;
}
//#CRESTRON_CHANGE_END
static gboolean
